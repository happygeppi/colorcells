...
class DJ {
  ...
  lines() {
    ...
    for (let vert of _verts) vert = vert;
  ...
  translate() {
    if (arguments.length == 1) {
      this.ctx.translate(arguments[0].x, arguments[0].y);
    } else {
      this.ctx.translate(arguments[0], arguments[1]);
    }
  }
  rotateX = (_a) => (this.rotAxes.x = _a);
  rotateY = (_a) => (this.rotAxes.y = _a);
  rotateZ = (_a) => (this.rotAxes.z = _a);
  rotate = (_a) => this.ctx.rotate(_a);
  rotatedX(_v, _a) {
    const _rotMat = [
      [1, 0, 0],
  ...
  project3d2d(_v3d, fov, eye, znear) {
  ...
  cancel() {
    dj.speed > 0 ? dj.speed-- : undefined;
    clearInterval(dj.bgFramecount);
  }
  addUpdater() {
    // has to be fixed
    dj.speed++;
    dj.bgFramecount = setInterval(BackgroundDraw, dj.framerate);
  }
  doIf(_cdts, _func) {
  ...
  setFramerate(_n) {
    ...
    this.framerate = _n;
    this.bgFramecount = setInterval(BackgroundDraw, 1000 / _n);
  }
  ...
  checkPerformance() {
    if (this.checkFR) {
      ...
      this.framerate = this.fixedFramerate
        ? this.framerate
        : 1000 / (this.recentFrames[1] - this.recentFrames[0]);
    }
  }
  start() {
    this.protoFuncs();
...
class ColorHSL {
  ...
  toRGB() {
    ...
    const _s = this.s / 100;
    const _l = this.l / 100;
    const _a = _s * Math.min(_l, 1 - _l);
    const _f = (_n, _k = (_n + _h / 30) % 12) =>
      Math.round((_l - _a * Math.max(Math.min(_k - 3, 9 - _k, 1), -1)) * 255);
    return [_f(0), _f(8), _f(4), this.a];
  }
}
class _Vector {
  constructor(_x, _y) {
  ...
  r() {
    return dj.dist(0, 0, this.x, this.y);
  }
  a() {
    return dj.atan(this.y, this.x);
  }
  add(_v) {
    this.x += _v.x;
    this.y += _v.y;
...
class _VectorMath {
  ...
  random() {
    ...
    else if (args.length == 1) _r = args[0];
...
function BackgroundDraw() {
  ...
  if (!dj.fixedFramerate)
    dj.bgFramecount = requestAnimationFrame(BackgroundDraw);
}

const $ = (id) => document.getElementById(id);
const $$ = (qSelector) => document.querySelector(qSelector);
const $$$ = (qSelector) => document.querySelectorAll(qSelector);
